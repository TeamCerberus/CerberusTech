...
public class ClassGeneratorUtil implements Constants {
	...
	public byte[] generateClass() {
		...
		if ( ! isInterface) {
			...
			generateField(BSHTHIS + className, "Lbeanshell/This;", ACC_PUBLIC, cw);

			// Generate the static bsh static reference holder field
			generateField(BSHSTATIC + className, "Lbeanshell/This;", ACC_PUBLIC + ACC_STATIC, cw);
		}
	...
	private static void generateMethod(String className, String fqClassName, String methodName, String returnType, String[] paramTypes, int modifiers, ClassWriter cw) {
		...
		if (isStatic) {
			cv.visitFieldInsn(GETSTATIC, fqClassName, BSHSTATIC + className, "Lbeanshell/This;");
		} else {
			// Push 'this'
			cv.visitVarInsn(ALOAD, 0);

			// Get the instance field
			cv.visitFieldInsn(GETFIELD, fqClassName, BSHTHIS + className, "Lbeanshell/This;");
		}
		...
		cv.visitMethodInsn(INVOKEVIRTUAL, "beanshell/This", "invokeMethod", Type.getMethodDescriptor(Type.getType(Object.class), new Type[]{Type.getType(String.class), Type.getType(Object[].class), Type.getType(Interpreter.class), Type.getType(CallStack.class), Type.getType(SimpleNode.class), Type.getType(Boolean.TYPE)}));

		// Generate code to unwrap bsh Primitive types
		cv.visitMethodInsn(INVOKESTATIC, "beanshell/Primitive", "unwrap", "(Ljava/lang/Object;)Ljava/lang/Object;");
	...
	void generateConstructor(int index, String[] paramTypes, int modifiers, ClassWriter cw) {
		...
		cv.visitMethodInsn(INVOKESTATIC, "beanshell/ClassGeneratorUtil", "initInstance", "(L" + GeneratedClass.class.getName().replace('.', '/') + ";Ljava/lang/String;[Ljava/lang/Object;)V");
	...
	void generateConstructorSwitch(int consIndex, int argsVar, int consArgsVar, CodeVisitor cv) {
		...
		cv.visitMethodInsn(INVOKESTATIC, "beanshell/ClassGeneratorUtil", "getConstructorArgs", "(Ljava/lang/String;Lbeanshell/This;[Ljava/lang/Object;I)" + "Lbeanshell/ClassGeneratorUtil$ConstructorArgs;");
		...
		cv.visitFieldInsn(GETFIELD, "beanshell/ClassGeneratorUtil$ConstructorArgs", "selector", "I");
	...
	private static void doSwitchBranch(int index, String targetClassName, String[] paramTypes, Label endLabel, Label[] labels, int consArgsVar, CodeVisitor cv) {
		...
		for (String type : paramTypes) {
			...
			String className = "beanshell/ClassGeneratorUtil$ConstructorArgs";
			String retType;
	...
	private static void generateParameterReifierCode(String[] paramTypes, boolean isStatic, final CodeVisitor cv) {
		...
		for (int i = 0; i < paramTypes.length; ++i) {
			...
			if (isPrimitive(param)) {
				...
				String type = "beanshell/Primitive";
	 ...
	 *
	 * @see beanshell.ClassGeneratorUtil#generateConstructor(int, String[], int, org.objectweb.asm.ClassWriter)
	 */
