...
public class GuiComputer extends GuiContainer {
	...
	public void initGui() {
		...
		buttonList.add(new GuiButton(1, j - 102, k + 10 + 20 + 5, 100, 20,
				"Power Off"));
		buttonList.add(new GuiButton(2, j - 102, k + 10 + 20 + 5 + 20 + 5, 100,
				20, "Close GUI"));
	}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2,
		...
		for (int x = 0; x < 200; x++) {
			for (int y = 0; y < 200; y++) {
				...
				pixel(var9, j + x + 11, k + y + 11, j + x + 1 + 11, k + y + 1
						+ 11, color);
		...
		while (Keyboard.next()) {
			...
			if (key != 0) {
				...
				boolean special = !Character.isLetterOrDigit(Character
						.toLowerCase(character));
				String name = special ? Keyboard.getKeyName(key) : Character
						.toString(character);
				if (name.equals("GRAVE")
						&& (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard
								.isKeyDown(Keyboard.KEY_RSHIFT))) {
					name = OSKeyboardLetters.At.letter; // Allows
				}
				// for
				// access
				// to
				...
				if (name.equals("GRAVE") || computer == null) {
					...
					ClientPacketHandler.sendComputerKeyboardEvent(
							computer.xCoord, computer.yCoord, computer.zCoord,
							computer.worldObj.provider.dimensionId,
							release ? OSKeyboardEvents.KeyPushed
									: OSKeyboardEvents.KeyReleased,
							OSKeyboardLetters.getFromLetter(name));
		...
		// OSMouseEvents event = Mouse.getEventButton() <= 0 ?
		// OSMouseEvents.Button : OSMouseEvents.Move;
		// }
		// }
	}
